...
let gearSvgNode = document.getElementById('gear');
...
window.addEventListener('scroll', () =>{
    ...
    dY = scrollY - previousPosition;
    previousPosition = scrollY;
    calculatePosition();
    let intensity = Math.abs(dY) > 15 ? 1 : Math.abs(dY)/15;
    gear.style.filter = `drop-shadow(0 0 2rem rgba(0, 255, 0, ${intensity}))`;
    timeoutID = setTimeout(()=>{
        gear.style.filter = `drop-shadow(0 0 2rem rgba(0, 255, 0, 0))`;
    }, 15)
